...
class Switches(config: LoginConfig, s3AsyncClient: S3AsyncClient) extends Loggable {
  ...
  def allSwitches: Map[String, SwitchState] =
    switchMapCache.get(ObjectId(config.switchBucket, fileName)).value.flatMap(_.toOption).flatten.getOrElse(Map.empty)
  ...
  def notifyIfSwitchStillActive(): Unit = {
    allSwitches.filter(_._2 == On).keys.foreach(notifier.sendStillActiveNotification)
  }
}
